 ...
 * DiskInode 类是虚拟磁盘上存储 Inode 的数据结构， 
 * 记录文件状态、 文件所属、 文件大小、 文件存储位置、 文件修改时间等信息。
 * 外存Inode位于文件存储设备上的外存Inode区中。每个文件有唯一对应的外存Inode，
 * 其作用是记录了该文件对应的控制信息。
...
public:
	...
	int		d_addr[10];		/* 用于文件逻辑块号和物理块号转换的基本索引表【这里用三级索引】 */
 ...
 * 每个内存inode通过外存inode所在存储设备的设备号(i_dev)以及该设备外存inode区
 * 中的编号(i_number)来确定其对应的外存inode。
 * 当文件被读写时， 首先需要将 DiskInode 中的信息对应复制到 Inode 中。
 */
class Inode
{
public:
...
public:
	...
	//short	i_dev;			/* 外存inode所在存储设备的设备号，【本实验只有一个磁盘，无需设备号】 */
