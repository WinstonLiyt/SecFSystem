 ...
 * SuperBlock 中除了存储了一些标志变量和计数变量以外，
 * 还管理着空闲盘块和空闲外存 inode 的分配使用。
 */
class SuperBlock
{
	/* Functions */
...
{
public:
	static const int SUPER_BLOCK_SECTOR_NUMBER = 0;		/* 定义SuperBlock位于磁盘上的扇区号，占据0，1两个扇区。【修改】 */

	static const int ROOTINO = 1;						/* 文件系统根目录外存Inode编号【修改】 */

	static const int INODE_NUMBER_PER_SECTOR = 8;		/* 外存INode对象长度为64字节，每个磁盘块可以存放512/64 = 8个外存Inode */
	static const int INODE_ZONE_START_SECTOR = 2;		/* 外存Inode区位于磁盘上的起始扇区号【修改】 */
	static const int INODE_ZONE_SIZE = 1024 - 2;		/* 磁盘上外存Inode区占据的扇区数【修改】 */
	static const int INODE_SIZE = 64;					/*外存Inode对象的长度*/
...
private:
	...
	SuperBlock* m_SuperBlock;			/* 指向全局SuperBlock */
};

extern FileSystem g_FileSystem;			/*  定义成全局变量负责整个文件系统管理 */
